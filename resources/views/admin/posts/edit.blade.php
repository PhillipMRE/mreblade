...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('sticky') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="sticky" value="0">
                    <input class="form-check-input" type="checkbox" name="sticky" id="sticky" value="1" {{ $post->sticky || old('sticky', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="sticky">{{ trans('cruds.post.fields.sticky') }}</label>
                </div>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.sticky_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="content">{{ trans('cruds.post.fields.content') }}</label>
                <textarea class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" name="content" id="content">{{ old('content', $post->content) }}</textarea>
            ...
            <div class="form-group">
                <label for="for">{{ trans('cruds.post.fields.for') }}</label>
                <input class="form-control {{ $errors->has('for') ? 'is-invalid' : '' }}" type="text" name="for" id="for" value="{{ old('for', $post->for) }}">
                @if($errors->has('for'))
                    <div class="invalid-feedback">
                        {{ $errors->first('for') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.post.fields.for_helper') }}</span>
            </div>
            <div class="form-group">
