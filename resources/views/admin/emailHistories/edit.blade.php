...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.emailHistory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.email-histories.update", [$emailHistory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="recipient_email">{{ trans('cruds.emailHistory.fields.recipient_email') }}</label>
                <input class="form-control {{ $errors->has('recipient_email') ? 'is-invalid' : '' }}" type="email" name="recipient_email" id="recipient_email" value="{{ old('recipient_email', $emailHistory->recipient_email) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sender_email') ? 'is-invalid' : '' }}" type="email" name="sender_email" id="sender_email" value="{{ old('sender_email', $emailHistory->sender_email) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('subject') ? 'is-invalid' : '' }}" type="text" name="subject" id="subject" value="{{ old('subject', $emailHistory->subject) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('state') ? 'is-invalid' : '' }}" type="text" name="state" id="state" value="{{ old('state', $emailHistory->state) }}">
                @if($errors->has('state'))
                    <div class="invalid-feedback">
                        {{ $errors->first('state') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('opens') ? 'is-invalid' : '' }}" type="text" name="opens" id="opens" value="{{ old('opens', $emailHistory->opens) }}">
                @if($errors->has('opens'))
                    <div class="invalid-feedback">
                        {{ $errors->first('opens') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('clicks') ? 'is-invalid' : '' }}" type="text" name="clicks" id="clicks" value="{{ old('clicks', $emailHistory->clicks) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('ts') ? 'is-invalid' : '' }}" type="text" name="ts" id="ts" value="{{ old('ts', $emailHistory->ts) }}">
                @if($errors->has('ts'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ts') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id">
                    @foreach($customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $emailHistory->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('msg') ? 'is-invalid' : '' }}" name="msg" id="msg">{{ old('msg', $emailHistory->msg) }}</textarea>
                @if($errors->has('msg'))
                    <div class="invalid-feedback">
                        {{ $errors->first('msg') }}
                    </div>
