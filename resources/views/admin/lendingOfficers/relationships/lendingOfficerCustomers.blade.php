...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-lendingOfficerCustomers">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($customers as $key => $customer)
                        <tr data-entry-id="{{ $customer->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $customer->active ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $customer->active ? 'checked' : '' }}>
                            </td>
                            <td>
                                <span style="display:none">{{ $customer->published ?? '' }}</span>
...
@endcan
  ...
  let table = $('.datatable-lendingOfficerCustomers:not(.ajaxTable)').DataTable({ buttons: dtButtons })
