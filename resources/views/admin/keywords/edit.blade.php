    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.keywords.update", [$keyword->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $keyword->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('template') ? 'is-invalid' : '' }}" name="template" id="template">{{ old('template', $keyword->template) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('map') ? 'is-invalid' : '' }}" name="map" id="map">{{ old('map', $keyword->map) }}</textarea>
                @if($errors->has('map'))
                    <div class="invalid-feedback">
                        {{ $errors->first('map') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('house_types') ? 'is-invalid' : '' }}" name="house_types" id="house_types">{{ old('house_types', $keyword->house_types) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('status_types') ? 'is-invalid' : '' }}" name="status_types" id="status_types">{{ old('status_types', $keyword->status_types) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shortcode') ? 'is-invalid' : '' }}" type="text" name="shortcode" id="shortcode" value="{{ old('shortcode', $keyword->shortcode) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('listing_settings') ? 'is-invalid' : '' }}" name="listing_settings" id="listing_settings">{{ old('listing_settings', $keyword->listing_settings) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sponsor_only') ? 'is-invalid' : '' }}" type="text" name="sponsor_only" id="sponsor_only" value="{{ old('sponsor_only', $keyword->sponsor_only) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agents') ? 'is-invalid' : '' }}" name="agents[]" id="agents" multiple>
                    @foreach($agents as $id => $agent)
                        <option value="{{ $id }}" {{ (in_array($id, old('agents', [])) || $keyword->agents->contains($id)) ? 'selected' : '' }}>{{ $agent }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('lending_officer') ? 'is-invalid' : '' }}" name="lending_officer_id" id="lending_officer_id">
                    @foreach($lending_officers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('lending_officer_id') ? old('lending_officer_id') : $keyword->lending_officer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
