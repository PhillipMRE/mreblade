        ...
        <form method="POST" action="{{ route("admin.sponsors.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('can_create_keyword') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="can_create_keyword" id="can_create_keyword" value="1" {{ old('can_create_keyword', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('carriers') ? 'is-invalid' : '' }}" name="carriers[]" id="carriers" multiple>
                    @foreach($carriers as $id => $carrier)
                        <option value="{{ $id }}" {{ in_array($id, old('carriers', [])) ? 'selected' : '' }}>{{ $carrier }}</option>
                    @endforeach
                </select>
