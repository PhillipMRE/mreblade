    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.phones.update", [$phone->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number') ? 'is-invalid' : '' }}" type="text" name="number" id="number" value="{{ old('number', $phone->number) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('phone_type') ? 'is-invalid' : '' }}" name="phone_type" id="phone_type">
                    ...
                    @foreach(App\Models\Phone::PHONE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('phone_type', $phone->phone_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('agent') ? 'is-invalid' : '' }}" name="agent_id" id="agent_id">
                    @foreach($agents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('agent_id') ? old('agent_id') : $phone->agent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('agent'))
                    <div class="invalid-feedback">
