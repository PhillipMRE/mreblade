    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('clients') ? 'is-invalid' : '' }}" name="clients[]" id="clients" multiple>
                    @foreach($clients as $id => $client)
                        <option value="{{ $id }}" {{ (in_array($id, old('clients', [])) || $customer->clients->contains($id)) ? 'selected' : '' }}>{{ $client }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fusebill') ? 'is-invalid' : '' }}" type="text" name="fusebill" id="fusebill" value="{{ old('fusebill', $customer->fusebill) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('settings') ? 'is-invalid' : '' }}" name="settings" id="settings">{{ old('settings', $customer->settings) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('website') ? 'is-invalid' : '' }}" name="website" id="website">{{ old('website', $customer->website) }}</textarea>
